...
export function LocationAutocomplete({ onSelect }: LocationAutocompleteProps) {
  ...
  useEffect(() => {
    ...
    if (
      lastValidQuery &&
      query.startsWith(lastValidQuery) &&
  ...
  return (
    <div ref={containerRef} className="relative w-full">
      <Input
        ...
        value={query}
        onChange={(e) => {
          // Si se empieza a escribir después de seleccionar una ubicación, resetear el estado
          if (isLocationSelected) {
        ...
        onFocus={() => {
          // Solo abrir si hay resultados o si no se ha seleccionado una ubicación
          if (!isLocationSelected && (results.length > 0 || query.length > 0)) {
            setIsOpen(true);
          }
        }}
        className="w-full"
              ...
              {results.map((location) => (
                <button
                  ...
                  onClick={() => {
                    ...
                    setIsLocationSelected(true); // Marcar que se ha seleccionado una ubicación
